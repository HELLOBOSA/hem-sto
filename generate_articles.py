#!/usr/bin/env python3
"""Generate 9 new SEO blog articles for Hemreform."""

import os
import shutil

TEMPLATE_PATH = '/home/user/hem-sto/blogg/badrumsrenovering-stockholm-checklista/index.html'
BLOGG_DIR = '/home/user/hem-sto/blogg'
BASE_URL = 'https://hemreform.se'
...
def make_article_block(article):
  ...
  <article class="article">
    <header class="article-hero wrap">
      ...
      <span class="eyebrow"><span class="tick" aria-hidden="true"></span><span data-en="Guide · Stockholm">Guide · Stockholm</span></span>
...
def build_page(article):
    ...
    # Title
    html = html.replace(
        '<title>Badrumsrenovering Stockholm: checklista innan start | Hemreform</title>',
        f'<title>{a["title"]}</title>'
    )

    # Meta description
    html = html.replace(
        '<meta name="description" content="Checklista inför badrumsrenovering i Stockholm: budget, ROT, material, tidsplan, offert och vad du bör förbereda innan arbetet startar.">',
        f'<meta name="description" content="{a["meta_desc"]}">'
    )

    # Canonical
    html = html.replace(
        '<link rel="canonical" href="https://hemreform.se/blogg/badrumsrenovering-stockholm-checklista/">',
        f'<link rel="canonical" href="{canonical}">'
    )

    # OG url
    html = html.replace(
        '<meta property="og:url" content="https://hemreform.se/blogg/badrumsrenovering-stockholm-checklista/">',
        f'<meta property="og:url" content="{canonical}">'
    )

    # OG title
    html = html.replace(
        '<meta property="og:title" content="Badrumsrenovering i Stockholm: checklista innan du börjar">',
        f'<meta property="og:title" content="{a["h1_sv"]}">'
    )

    # OG description
    html = html.replace(
        '<meta property="og:description" content="Checklista inför badrumsrenovering i Stockholm: budget, ROT, material, tidsplan, offert och vad du bör förbereda innan arbetet startar.">',
        f'<meta property="og:description" content="{a["meta_desc"]}">'
    )

    # OG image
    html = html.replace(
        '<meta property="og:image" content="https://hemreform.se/images/projekt/apartment-v1-after.webp">',
        f'<meta property="og:image" content="{BASE_URL}{a["image_src"]}">'
    )

    # Schema JSON-LD replacements
    html = html.replace(
        '"headline": "Badrumsrenovering i Stockholm: checklista innan du börjar"',
        f'"headline": "{a["h1_sv"]}"'
    )
    html = html.replace(
        '"description": "Checklista inför badrumsrenovering i Stockholm: budget, ROT, material, tidsplan, offert och vad du bör förbereda innan arbetet startar."',
        f'"description": "{a["meta_desc"]}"'
    )
    html = html.replace(
        '"image": "https://hemreform.se/images/projekt/apartment-v1-after.webp"',
        f'"image": "{BASE_URL}{a["image_src"]}"'
    )

    # Schema mainEntityOfPage
    html = html.replace(
        '"mainEntityOfPage": "https://hemreform.se/blogg/badrumsrenovering-stockholm-checklista/"',
...
# 1. badrumsrenovering-stockholm-pris
{
    'slug': 'badrumsrenovering-stockholm-pris',
    'title': 'Badrumsrenovering Stockholm pris 2026 | Kostnad & ROT | Hemreform',
    'meta_desc': 'Vad kostar en badrumsrenovering i Stockholm 2026? Prisintervall per storlek, vad som driver kostnaden och hur ROT-avdraget påverkar slutsumman.',
    'h1_sv': 'Vad kostar en badrumsrenovering i Stockholm?',
    'h1_en': 'What does a bathroom renovation cost in Stockholm?',
    'intro_sv': 'Priset på en badrumsrenovering i Stockholm varierar kraftigt beroende på storlek, befintligt skick och vald standard. Den här guiden ger konkreta prisintervall, förklarar vad som driver kostnaden och visar hur ROT-avdraget 2026 påverkar vad du faktiskt betalar.',
    'intro_en': 'The cost of a bathroom renovation in Stockholm varies significantly depending on size, existing condition and chosen standard. This guide gives concrete price ranges, explains what drives the cost and shows how the 2026 ROT deduction affects what you actually pay.',
    'image_src': '/images/projekt/apartment-k38-after.webp',
    'image_w': '1800',
    'image_h': '1344',
    'image_alt_sv': 'Renoverat badrum i Stockholm — Apartment K38',
    'body_sections': [
        {'type': 'h2', 'sv': 'Vad kostar ett badrum att renovera i Stockholm?', 'en': 'What does it cost to renovate a bathroom in Stockholm?'},
        ...
        {'type': 'h2', 'sv': 'Vanliga frågor om badrumsrenovering i Stockholm', 'en': 'Common questions about bathroom renovation in Stockholm'},
    ...
    'cta_h2_sv': 'Planerar du badrumsrenovering i Stockholm?',
    'cta_h2_en': 'Planning a bathroom renovation in Stockholm?',
    ...
    'pager_prev': {'url': '/blogg/badrumsrenovering-stockholm-checklista/', 'sv': 'Badrumsrenovering i Stockholm: checklista innan du börjar', 'en': 'Bathroom renovation in Stockholm: checklist before you start'},
    'pager_next': {'url': '/blogg/koksrenovering-stockholm-pris/', 'sv': 'Köksrenovering i Stockholm: pris, materialval och smart budget 2026', 'en': 'Kitchen renovation in Stockholm: price, materials and smart budget 2026'},
},

# 2. koksrenovering-stockholm-pris
{
    'slug': 'koksrenovering-stockholm-pris',
    'title': 'Köksrenovering Stockholm pris 2026 | Kostnad & budgettips | Hemreform',
    'meta_desc': 'Prisguide för köksrenovering i Stockholm. Jämför budget, standard och platsbyggda lösningar samt vad el, VVS och material påverkar slutkostnaden.',
    'h1_sv': 'Köksrenovering i Stockholm: pris, materialval och smart budget',
    'h1_en': 'Kitchen renovation in Stockholm: price, materials and smart budget',
    'intro_sv': 'En köksrenovering i Stockholm kan kosta allt från 60 000 kronor för en kosmetisk uppfräschning till över 400 000 kronor för ett helt nytt kök med platsbyggda lösningar. Den här guiden ger dig konkreta prisintervall och hjälper dig att förstå vad som driver kostnaden.',
    'intro_en': 'A kitchen renovation in Stockholm can cost anything from 60,000 kronor for a cosmetic refresh to over 400,000 kronor for a completely new kitchen with custom-built solutions. This guide gives you concrete price ranges and helps you understand what drives the cost.',
    'image_src': '/images/projekt/apartment-k12.webp',
    'image_w': '1800',
    'image_h': '798',
    'image_alt_sv': 'Renoverat kök i Stockholm — Apartment K12',
    'body_sections': [
        {'type': 'h2', 'sv': 'Vad kostar en köksrenovering i Stockholm?', 'en': 'What does a kitchen renovation cost in Stockholm?'},
        ...
        {'type': 'p', 'sv': 'Platsbyggda kök passar när standardmåtten inte passar — i sekelskifteslägenheter med snedväggar, i äldre lägenheter med oregelbundna mått eller när du vill utnyttja varje centimeter. Det är också det bästa alternativet när du vill ha ett genomgående formspråk som löper från kök till matplats och hall. Hemreform tillverkar och monterar platsbyggda köksmoduler direkt i din bostad i Stockholm.', 'en': 'Custom-built kitchens are suitable when standard measurements do not fit — in sekelskifteslägenheter with slanted walls, in older apartments with irregular dimensions or when you want to use every centimetre. It is also the best option when you want a consistent design language running from kitchen to dining area and hallway. Hemreform manufactures and installs custom kitchen modules directly in your home in Stockholm.'},
    ...
    'cta_h2_sv': 'Planerar du köksrenovering i Stockholm?',
    'cta_h2_en': 'Planning a kitchen renovation in Stockholm?',
    ...
    'pager_prev': {'url': '/blogg/badrumsrenovering-stockholm-pris/', 'sv': 'Vad kostar en badrumsrenovering i Stockholm?', 'en': 'What does a bathroom renovation cost in Stockholm?'},
    'pager_next': {'url': '/blogg/lagenhetsrenovering-stockholm/', 'sv': 'Lägenhetsrenovering i Stockholm: kostnad, tidsplan och BRF-regler', 'en': 'Apartment renovation in Stockholm: cost, timeline and BRF rules'},
},

# 3. lagenhetsrenovering-stockholm
{
    'slug': 'lagenhetsrenovering-stockholm',
    'title': 'Lägenhetsrenovering Stockholm | Pris, BRF-regler & tidsplan | Hemreform',
    'meta_desc': 'Planerar du lägenhetsrenovering i Stockholm? Läs om kostnad, steg, BRF-godkännande, tidsplan och vanliga misstag innan du tar in offert.',
    'h1_sv': 'Lägenhetsrenovering i Stockholm: kostnad, tidsplan och BRF-regler',
    'h1_en': 'Apartment renovation in Stockholm: cost, timeline and BRF rules',
    'intro_sv': 'Att renovera en lägenhet i Stockholm kräver planering på tre nivåer: budget och materialval, en realistisk tidsplan och förståelse för de regler som gäller i din bostadsrättsförening. Den här guiden täcker alla tre.',
    'intro_en': 'Renovating an apartment in Stockholm requires planning at three levels: budget and material choices, a realistic timeline and an understanding of the rules in your housing cooperative. This guide covers all three.',
    'image_src': '/images/projekt/apartment-a75-after.webp',
    'image_w': '1800',
    'image_h': '1263',
    'image_alt_sv': 'Renoverad lägenhet i Stockholm — Apartment A75',
    'body_sections': [
        {'type': 'h2', 'sv': 'Vad kostar en lägenhetsrenovering i Stockholm?', 'en': 'What does an apartment renovation cost in Stockholm?'},
    ...
    'pager_prev': {'url': '/blogg/koksrenovering-stockholm-pris/', 'sv': 'Köksrenovering i Stockholm: pris, materialval och smart budget', 'en': 'Kitchen renovation in Stockholm: price, materials and smart budget'},
    'pager_next': {'url': '/blogg/totalrenovering-stockholm-pris/', 'sv': 'Totalrenovering i Stockholm: pris per kvm, process och vanliga misstag', 'en': 'Total renovation in Stockholm: price per sqm, process and common mistakes'},
},

# 4. totalrenovering-stockholm-pris
{
    'slug': 'totalrenovering-stockholm-pris',
    'title': 'Totalrenovering Stockholm | Pris per kvm & process | Hemreform',
    'meta_desc': 'Så planerar du en totalrenovering i Stockholm. Se pris per kvm, process, tidslinje och vilka kostnadsfällor du bör undvika.',
    'h1_sv': 'Totalrenovering i Stockholm: pris per kvm, process och vanliga misstag',
    'h1_en': 'Total renovation in Stockholm: price per sqm, process and common mistakes',
    'intro_sv': 'En totalrenovering i Stockholm innebär att bostaden eller lokalen renoveras från grunden — badrum, kök, ytskikt, förvaring, el och VVS i ett sammanhängande projekt. Det kräver noggrann planering, rätt yrkeskompetens och en tydlig projektstyrning.',
    'intro_en': 'A total renovation in Stockholm means the property is renovated from scratch — bathroom, kitchen, surfaces, storage, electrical and plumbing in one cohesive project. It requires careful planning, the right professional expertise and clear project management.',
    'image_src': '/images/projekt/villa-boa-after.webp',
    'image_w': '1800',
    'image_h': '1004',
    'image_alt_sv': 'Totalrenovering i Stockholm — Villa Boa',
    'body_sections': [
        {'type': 'h2', 'sv': 'Vad kostar en totalrenovering per kvm i Stockholm?', 'en': 'What does a total renovation cost per sqm in Stockholm?'},
        {'type': 'p', 'sv': 'En totalrenovering av en lägenhet kostar normalt 8 000–20 000 kronor per kvm beroende på standard, byggnadens ålder och projektets komplexitet. En enklare renovering av en modern lägenhet hamnar i det lägre intervallet. En renovering av ett äldre sekelskifteslägenheter med fuktskador, gamla installationer och kulturhistoriska krav kan hamna i det övre. En villa med tillbyggnad och nytt kök och badrum kostar typiskt 2–5 miljoner kronor totalt.', 'en': 'A total apartment renovation normally costs 8,000–20,000 kronor per sqm depending on standard, building age and project complexity. A simpler renovation of a modern apartment lands in the lower range. Renovating an older sekelskifteslägenheter with moisture damage, old installations and heritage requirements can land in the upper range. A house with extension and new kitchen and bathroom typically costs 2–5 million kronor in total.'},
        ...
        {'type': 'p', 'sv': 'De vanligaste kostnadsfällorna är: att låsa materialval för sent (leveranstider påverkar hela tidsplanen), att inte besikta ordentligt innan start (dolda fuktskador och asbest är vanligt i äldre Stockholmsbyggnader), att inte ha en tydlig ansvarsfördelning mellan hantverkarna, att sakna en byggledare som håller ihop projektet och att inte budgetera 10–15 procent för oförutsedda kostnader.', 'en': 'The most common cost traps are: locking material choices too late (delivery times affect the entire timeline), not inspecting properly before start (hidden moisture damage and asbestos is common in older Stockholm buildings), not having clear responsibility allocation between trades, lacking a site manager to coordinate the project and not budgeting 10–15 percent for unforeseen costs.'},
    ...
    'cta_h2_sv': 'Planerar du en totalrenovering i Stockholm?',
    'cta_h2_en': 'Planning a total renovation in Stockholm?',
    ...
    'pager_prev': {'url': '/blogg/lagenhetsrenovering-stockholm/', 'sv': 'Lägenhetsrenovering i Stockholm: kostnad, tidsplan och BRF-regler', 'en': 'Apartment renovation in Stockholm: cost, timeline and BRF rules'},
    'pager_next': {'url': '/blogg/bygglov-renovering-stockholm/', 'sv': 'Behöver du bygglov eller anmälan vid renovering i Stockholm?', 'en': 'Do you need a building permit or notification for renovation in Stockholm?'},
},

# 5. bygglov-renovering-stockholm
{
    'slug': 'bygglov-renovering-stockholm',
    'title': 'Bygglov eller anmälan vid renovering i Stockholm? | Hemreform',
    'meta_desc': 'Behöver du bygglov eller anmälan vid renovering i Stockholm? Vi går igenom planlösning, bärande väggar, ventilation, VVS och startbesked.',
    'h1_sv': 'Behöver du bygglov eller anmälan vid renovering i Stockholm?',
    'h1_en': 'Do you need a building permit or notification for renovation in Stockholm?',
    'intro_sv': 'Många renoveringsåtgärder i Stockholm kräver varken bygglov eller anmälan — men vissa ingrepp gör det. Att förstå skillnaden kan spara dig tid, pengar och juridiska problem. Den här guiden ger ett klart beslutsunderlag.',
    'intro_en': 'Many renovation measures in Stockholm require neither a building permit nor a notification — but some do. Understanding the difference can save you time, money and legal problems. This guide gives clear decision-making guidance.',
    'image_src': '/images/projekt/norrsken-office-after.webp',
    'image_w': '1800',
    'image_h': '982',
    'image_alt_sv': 'Kommersiell renovering i Stockholm — Norrsken Office',
    'body_sections': [
        ...
        {'type': 'h2', 'sv': 'När krävs en anmälan till Stockholm stads stadsbyggnadsförvaltning?', 'en': 'When is a notification to Stockholm\'s city planning department required?'},
        ...
        {'type': 'p', 'sv': 'Ett bygglov krävs vid åtgärder som ändrar byggnadens yttre utseende i ett detaljplanelagt område, tillbyggnader som ökar byggnadsarean, ändring av byggnadstyp (t.ex. från bostad till lokal), nybyggnation och vid fasadändringar i känsliga kulturhistoriska miljöer. I Stockholms innerstad och i sekelskifteslägenheterkvarter gäller ofta utökade krav.', 'en': 'A building permit is required when measures change the building\'s exterior appearance in a detailed plan area, extensions increasing the floor area, changes of building type (e.g. from residential to commercial), new construction and facade changes in sensitive heritage environments. In Stockholm\'s inner city and sekelskifteslägenheter blocks, extended requirements often apply.'},
        ...
        {'type': 'h2', 'sv': 'Hur ansöker du om bygglov eller anmälan i Stockholm?', 'en': 'How do you apply for a building permit or notification in Stockholm?'},
        {'type': 'p', 'sv': 'Ansökan görs via Stockholms Stads e-tjänst för bygglov. Du behöver oftast bifoga situationsplan, fasadritningar och planritningar. Handläggningstiden är normalt 4–10 veckor för ett bygglov och 2–4 veckor för en anmälan. Hemreform kan hjälpa till med dokumentationen och koordinera ansökan som en del av projektet.', 'en': 'Applications are made through Stockholm City\'s e-service for building permits. You normally need to attach a site plan, facade drawings and floor plans. Processing time is normally 4–10 weeks for a building permit and 2–4 weeks for a notification. Hemreform can assist with documentation and coordinate the application as part of the project.'},
    ],
    ...
    'pager_prev': {'url': '/blogg/totalrenovering-stockholm-pris/', 'sv': 'Totalrenovering i Stockholm: pris per kvm, process och vanliga misstag', 'en': 'Total renovation in Stockholm: price per sqm, process and common mistakes'},
    'pager_next': {'url': '/blogg/rot-avdrag-2026-stockholm/', 'sv': 'ROT-avdrag för renovering 2026: så fungerar det i Stockholm', 'en': 'ROT deduction for renovation 2026: how it works in Stockholm'},
},

# 6. rot-avdrag-2026-stockholm
{
    'slug': 'rot-avdrag-2026-stockholm',
    ...
    'h1_sv': 'ROT-avdrag för renovering 2026: så fungerar det i Stockholm',
    'h1_en': 'ROT deduction for renovation 2026: how it works in Stockholm',
    ...
    'image_w': '1800',
    'image_h': '1004',
    'image_alt_sv': 'Renovering med ROT-avdrag i Stockholm — Apartment V1',
    'body_sections': [
    ...
    'pager_prev': {'url': '/blogg/bygglov-renovering-stockholm/', 'sv': 'Behöver du bygglov eller anmälan vid renovering i Stockholm?', 'en': 'Do you need a building permit or notification for renovation in Stockholm?'},
    'pager_next': {'url': '/blogg/badrumsrenovering-bostadsratt-stockholm/', 'sv': 'Badrumsrenovering i bostadsrätt i Stockholm: regler, intyg och steg för steg', 'en': 'Bathroom renovation in a housing cooperative in Stockholm: rules, certificates and step by step'},
},

# 7. badrumsrenovering-bostadsratt-stockholm
{
    'slug': 'badrumsrenovering-bostadsratt-stockholm',
    'title': 'Badrumsrenovering bostadsrätt Stockholm | Regler & intyg | Hemreform',
    'meta_desc': 'Renovera badrum i bostadsrätt i Stockholm? Här är reglerna, intygen och checklistan för BRF, BKR, Säker Vatten och el.',
    'h1_sv': 'Badrumsrenovering i bostadsrätt i Stockholm: regler, intyg och steg för steg',
    'h1_en': 'Bathroom renovation in a housing cooperative in Stockholm: rules, certificates and step by step',
    'intro_sv': 'Att renovera badrummet i en bostadsrätt i Stockholm kräver mer förberedelse än en vanlig badrumsrenovering. BRF-regler, intyg för tätskikt och el, och styrelsens tillstånd är alla delar av processen — och om du hoppar över dem kan det bli kostsamt.',
    'intro_en': 'Renovating a bathroom in a housing cooperative in Stockholm requires more preparation than a regular bathroom renovation. BRF rules, certificates for waterproofing and electrical work, and board approval are all part of the process — and skipping them can be costly.',
    'image_src': '/images/projekt/bistro-after.webp',
    'image_w': '1208',
    'image_h': '1800',
    'image_alt_sv': 'Renovering i Stockholm — Hemreform projektarkiv',
    'body_sections': [
        ...
        {'type': 'h2', 'sv': 'Vad kostar ett BRF-badrum i Stockholm?', 'en': 'What does a BRF bathroom cost in Stockholm?'},
    ...
    'pager_prev': {'url': '/blogg/rot-avdrag-2026-stockholm/', 'sv': 'ROT-avdrag för renovering 2026: så fungerar det i Stockholm', 'en': 'ROT deduction for renovation 2026: how it works in Stockholm'},
    'pager_next': {'url': '/blogg/koksrenovering-bostadsratt-stockholm/', 'sv': 'Köksrenovering i bostadsrätt i Stockholm: ventilation, el, VVS och styrelsens krav', 'en': 'Kitchen renovation in a housing cooperative in Stockholm: ventilation, electrical, plumbing and board requirements'},
},

# 8. koksrenovering-bostadsratt-stockholm
{
    'slug': 'koksrenovering-bostadsratt-stockholm',
    'title': 'Köksrenovering bostadsrätt Stockholm | Regler för BRF | Hemreform',
    'meta_desc': 'Guide till köksrenovering i bostadsrätt i Stockholm. Läs om BRF-regler, ventilation, el, VVS, tidsplan och smart köksplanering.',
    'h1_sv': 'Köksrenovering i bostadsrätt i Stockholm: ventilation, el, VVS och styrelsens krav',
    'h1_en': 'Kitchen renovation in a housing cooperative in Stockholm: ventilation, electrical, plumbing and board requirements',
    ...
    'image_w': '1344',
    'image_h': '1800',
    'image_alt_sv': 'Köksrenovering i bostadsrätt Stockholm — Apartment K38',
    'body_sections': [
    ...
    'pager_prev': {'url': '/blogg/badrumsrenovering-bostadsratt-stockholm/', 'sv': 'Badrumsrenovering i bostadsrätt i Stockholm: regler, intyg och steg för steg', 'en': 'Bathroom renovation in a housing cooperative in Stockholm: rules, certificates and step by step'},
    'pager_next': {'url': '/blogg/specialsnickeri-stockholm/', 'sv': 'Specialsnickeri i Stockholm: platsbyggda garderober, bokhyllor och smart förvaring', 'en': 'Custom carpentry in Stockholm: built-in wardrobes, bookshelves and smart storage'},
},

# 9. specialsnickeri-stockholm
{
    'slug': 'specialsnickeri-stockholm',
    'title': 'Specialsnickeri Stockholm | Platsbyggd förvaring & garderober | Hemreform',
    'meta_desc': 'Behöver du specialsnickeri i Stockholm? Läs om platsbyggda garderober, bokhyllor och måttanpassad förvaring för äldre lägenheter och sekelskifteslägenheter.',
    'h1_sv': 'Specialsnickeri i Stockholm: platsbyggda garderober, bokhyllor och smart förvaring',
    'h1_en': 'Custom carpentry in Stockholm: built-in wardrobes, bookshelves and smart storage',
    'intro_sv': 'I Stockholms sekelskifteslägenheter, sekelskifteslägenheter och moderna bostäder finns sällan en standard lösning som passar. Platsbyggda garderober, bokhyllor och förvaring gör att varje centimeter används rätt — och att formspråket håller ihop hela bostaden.',
    'intro_en': 'In Stockholm\'s sekelskifteslägenheter, turn-of-the-century apartments and modern homes there is rarely a standard solution that fits. Built-in wardrobes, bookshelves and custom storage means every centimetre is used correctly — and that the design language holds the entire home together.',
    'image_src': '/images/projekt/apartment-a75.webp',
    'image_w': '1800',
    'image_h': '1344',
    'image_alt_sv': 'Platsbyggd förvaring i Stockholm — Hemreform specialsnickeri',
    'body_sections': [
        ...
        {'type': 'h2', 'sv': 'Vad kostar specialsnickeri i Stockholm?', 'en': 'What does custom carpentry cost in Stockholm?'},
    ...
    'pager_prev': {'url': '/blogg/koksrenovering-bostadsratt-stockholm/', 'sv': 'Köksrenovering i bostadsrätt i Stockholm: ventilation, el, VVS och styrelsens krav', 'en': 'Kitchen renovation in a housing cooperative in Stockholm: ventilation, electrical, plumbing and board requirements'},
    'pager_next': {'url': '/blogg/badrumsrenovering-stockholm-checklista/', 'sv': 'Badrumsrenovering i Stockholm: checklista innan du börjar', 'en': 'Bathroom renovation in Stockholm: checklist before you start'},
},
